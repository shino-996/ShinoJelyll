...
接着要进行第二次回调, 即`nextAsync.execute()`. 因为我们只知道第二次回调前要对第一次回调的结果做什么（就是闭包`transform`里所定义的）, 而不知道第三次回调前要对第二次回调的结果做什么（因为还没有对第二次回调执行`flatMap`方法）, 所以`nextAsync.execute()`的参数是`flatMap`返回值的`Async`的回调函数`nextCallback`. 
...
![](/source/2017-02-10-使用函数式swift解决回调地狱-value.png)

而将一个**值**转换为另一个**值**的**函数**就是这样的

![](/source/2017-02-10-使用函数式swift解决回调地狱-value_apply.png)

有的时候, 我们会将**值**进行**封装**, 将其称为**数据类型**

![](/source/2017-02-10-使用函数式swift解决回调地狱-value_and_context.png)

而将一个**值**转换为另一个**值**的函数无法在**封装后的值**上起作用

![](/source/2017-02-10-使用函数式swift解决回调地狱-no_fmap_ouch.png)

这时我们就需要一种方法, 可以将原来只能作用在值上的函数作用在封装后的数据类型上, 它的实现应该是这样的：

![](/source/2017-02-10-使用函数式swift解决回调地狱-fmap_just.png)
...
![](/source/2017-02-10-使用函数式swift解决回调地狱-half.png)

而这样的函数也不能直接运用在数据类型上, 因为输入变成了数据类型, 而不是裸露的值：

![](/source/2017-02-10-使用函数式swift解决回调地狱-half_ouch.png)
